    ...
    public func validateField() -> ValidationError? {
        return rules.filter{
            ...
            if field is UITextField {
                let tfField = field as! UITextField
                validationString = tfField.validationText
            } else if field is UITextView {
                let tvField = field as! UITextView
                validationString = tvField.validationText
            }
            return !$0.validate(validationString)
        }.map{
            rule -> ValidationError in return ValidationError(field: self.field, errorLabel: self.errorLabel, error: rule.errorMessage())
        }.first
    }
}
